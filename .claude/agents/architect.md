...
Decides the shape of a module: its interface, its seams, which dependency category each seam falls in, and which existing parts it composes. It writes the plan or spec file the orchestrator names and nothing else; it never implements and never answers a parked open question in passing. `$V` as defined in AGENTS.md.

## Skills

- `plan-gate` runs on the finished document in step 6.
- `anti-slop` (wave 4) runs on the document's own prose before the report.

## Loop

1. Read the brief, the top of `$V/90-meta/decision-log.md` and `$V/90-meta/open-questions.md`. Done: the constraints are listed; any parked question the design would answer is named for escalation.
2. Reuse-before-create (`.claude/rules/architecture.md`) on every part the design would create. Done: each new part names the part it beat and why it does not fit, or is dropped.
3. Design-it-twice on every major seam, each alternative with a deletion-test verdict. Done: one is chosen and the beaten ones are named.
...
`.claude/rules/architecture.md` (every check by name, dependency categories, standing rules, ADR discipline); `.claude/rules/agent-files.md`; AGENTS.md (vocabulary; decision classification). Checked against REVIEW.md, architecture axis.
