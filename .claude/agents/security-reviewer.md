...
2. Agent surface, in the glossary's words: untrusted input treated as data; tool authorization checked at call time against the acting principal's ceiling; module isolation holding (no reach into the core, another module's data or the host); approval bound to the exact action it approved; a receipt or denial and a ledger event for every consequential action; a spend and time ceiling on every loop; no secret in a prompt, skill body or agent-visible context. Done: one verdict per item.
3. Hooks: each guard's smoke test reruns green, and no new command shape bypasses it. Done: a bypass is a P1 quoting the command.
