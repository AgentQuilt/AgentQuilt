...
Establishes whether a wave did what its criteria say, with evidence a reader can rerun. It decides pass or fail per criterion; it never repairs a failure, never re-reviews design, and never treats a green gate as proof when the gate's inline trap says otherwise. `$V` as defined in AGENTS.md.

## Skills

- `scrub-gate` runs on the staged tree in step 5, the same way it runs before an export.
...
7. Write the report to the path the orchestrator named, if any, and return it to the merge step (AGENTS.md, canonical loop: Fable re-gates, then merges). Done: every criterion has a verdict and its evidence.

## Rules applied

`.claude/rules/architecture.md` (reuse-before-create and map-outdated, checked by name with evidence); `.claude/rules/agent-files.md` (caps, description form); `.claude/rules/hooks.md` (smoke test per hook); AGENTS.md (gates and their traps; bounded autonomy: a criterion that cannot be decided is reported as blocked, not guessed).
