...
1. Read the file. An entry is open when it ends `→ *open*`; count with `grep -c '→ \*open\*$'` over this file and the inbox, minus each header's one example marker; an entry carrying no `→ *` marker at all is malformed and is read by eye, not counted. Done: the open count is stated, `0` included.
