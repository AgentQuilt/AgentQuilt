...
1. Read the file. An entry is open when it ends `→ *open*`; count with `grep -c '→ \*open\*$'` over this file and the inbox, minus each header's one example marker; an entry carrying no `→ *` marker at all is malformed and is read by eye, not counted. While `.claude/.curate/RUNNING` exists a zero count is not evidence — a detached lane may be mid-write; wait for it to clear or re-count after `last_curate.txt` moves. Done: the open count is stated, `0` included.
2. Locate entries by the same end-anchored marker step 1 counts with, never by matching marker text or bullet shape inside an entry (entries here quote both); a read that yields fewer entries than the count is a parse failure resolved by eye, not an empty file. For each open entry, decide in one sentence: accept when the change follows from the evidence and the target file is the rule's one home; reject when the evidence is one session, one error string or a rule already stated elsewhere; defer when it needs information the entry lacks, naming what. Cited earlier entries on the same item weigh in favour of accepting. Done: every open entry has a sentence.
3. Route accepted entries. One line of change (a cap number, a name, a by-name reference): the orchestrator folds it directly into the running wave's branch, or, with no wave running, into the next wave's first commit; either way it is named in that wave's context file, so Codex sees it. Anything larger: a task line for the running or next wave; past a line's worth of design, a task in the plan through `plan-gate`. The change itself lands through the wave loop; this skill edits no factory file. A fold that edits a `SKILL.md` updates that skill's `[touched:]` date in `.claude/skills/INDEX.md` in the same commit. Done: every accepted entry names the fold, wave or plan that carries it.
4. Mark each decided entry in place, `→ *<decision> (YYYY-MM-DD): <sentence>*`, replacing only the marker at the end of the entry's line — marker text quoted inside the entry stays untouched — and delete nothing. Done: no open entry from step 1 remains.
