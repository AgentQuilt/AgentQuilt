...
1. Scan at the sink: run `bash .claude/skills/scrub-gate/scripts/scrub.sh [paths]` on the files that are actually passed on (the staged tree, the cherry-pick, the file handed to a web search), never on a re-rendered string. Default with no paths: every tracked file. Done: exit 0, or every hit listed as `file:line`.
