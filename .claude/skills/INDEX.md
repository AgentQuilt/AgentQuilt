...
- anti-slop — The implementer's self-check on its own diff before the report: reuse-before-create and the architecture checks by name, stop conditions, `Done:`/`Left out:`; `references/` carry the pattern catalogue, the prose routing to the humanizer chain, and the measures plus the ledger entry. Implementer-fired on its own diff, plus the closing anti-slop pass each wave (Fable); never on prose. [touched: 2026-08-24]
