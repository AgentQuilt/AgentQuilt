...
One example per line. `REVIEW.md` sets severity and wins where the two disagree.
...
- reuse-before-create unmet: a new part with no closest-existing-part sentence in the change description.
- one-adapter-seam: a port with one adapter.
...
- The same shape everywhere: a docstring on every function including trivial ones, identical try/except in every route, the same five-line preamble in every module.
...
- Inflated line count: the diff grows by scaffold, re-exports, `__init__` boilerplate or docstrings on trivial functions, not by the change.
...
- Poor locality: a function that reads three fields of another module's object to compute what that module should expose. Move the computation next to the data; in Python, a module function beside it, not a new class.
...
If a gate is red, fix the code; a `pyproject.toml` change is a reviewable diff.
