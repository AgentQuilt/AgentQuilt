...
7. Cross to the public repo only after the green light: generalise internal names on the source page first (hosts, paths, machine names), then `npx --no-install lavish-axi export .lavish/<name>.html --out docs/plans/<slug>/index.html`, then `scrub-gate` on the exported files, then the by-eye read. A hit is fixed on the source page and re-exported, never on the export. Done: `scrub.sh` exits 0 on the files that cross and `docs/plans/index.md` lists the plan.
