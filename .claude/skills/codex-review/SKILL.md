...
2. Run anchored to the wave checkout — an absolute `cd` to the worktree in the same command, never a bare relative invocation, because the session cwd drifts (AGENTS.md, Git and branches): `cd <worktree-absolute-path> && bash .claude/skills/codex-review/scripts/review.sh diff <wave> [base] [round]` or `... plan <wave> <file> [round]`; prompt and output land in `temp/`. Done: a `VERDICT:` line printed under `--- gate ---`.
...
4. Fold: first check each finding against the wave's acceptance criteria — one that adds a file, a seam or behaviour outside them is a User Challenge for the owner, never folded on the orchestrator's judgment; then every P1, and every P2 unless argued away in one sentence in the next context file. Done: the AGENTS.md gates pass on the folded tree, then one commit per round with the round number in the message; a red gate is fixed inside the round, never carried into the next. A disposition never downgrades a finding on evidence from a build that predates the fold: evidence names its build, or it is not evidence. A fold is verified landed — grep one sentinel from each edit in the artefact, the sentinel a short single-line token that cannot wrap, checked over whitespace-normalised text — before the next round launches; an edit script that writes only after all its replacements chains the review with `&&`, or the round is spent reviewing the unfolded artefact.
