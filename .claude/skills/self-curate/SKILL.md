...
5. Apply: `Edit` over `Write`; YAML front-matter intact; wiki-links vault-internal (`[[90-meta/decision-log]]`) and resolving. Nothing is deleted from the decision log, open questions, `$V/10-executive/` or `$V/docs/`: append, edit in place or move, and mark superseded text as superseded. No secret, token or `.env` value goes anywhere. Done: `comm -13 <(echo "$BEFORE") <(git status --porcelain | sort)` names no factory file (changes present before the run do not count).
...
7. Rotate and re-stamp. Only the `N` lines read in step 1 are processed; lines appended since stay live (neither hook rotates, this step does). A hook-spawned lane archives all `N`; an in-session lane archives only its own session's lines among them and keeps foreign ones live (`SID` is this session's id from step 1):
   ```
   J=.claude/.curate/journal.jsonl; A=".claude/.curate/archive/journal-$(date -u +%Y%m%dT%H%M%SZ).jsonl"; mkdir -p .claude/.curate/archive
   head -n "$N" "$J" | grep -F "$SID" > "$A"; { head -n "$N" "$J" | grep -vF "$SID"; tail -n +$((N+1)) "$J"; } > "$J.tmp" && mv "$J.tmp" "$J"   # hook-spawned lane: SID=. (matches every line)
