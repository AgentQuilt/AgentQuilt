...
1. Read: `.claude/.curate/journal.jsonl` (snapshot what was read: `SNAP=$(mktemp); cp .claude/.curate/journal.jsonl "$SNAP"`), `.claude/.curate/last_curate.txt` (keep its value as `START_STAMP`), `git status --porcelain` (keep it: `BEFORE=$(git status --porcelain | sort)`) and `git diff --stat`, today's `$V/90-meta/session-log/YYYY-MM-DD.md`, `head -40` of `$V/90-meta/decision-log.md` and `$V/90-meta/open-questions.md`, `$V/Home.md`. Then pick the lane type:
