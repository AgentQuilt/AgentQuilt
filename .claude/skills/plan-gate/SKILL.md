...
4. Acceptance criteria. Numbered, each pass/fail by a command or a read: "the gate works" fails the test; "three synthetic outputs return FAIL/FAIL/PASS" passes it. For every wave the criteria answer "where does a person see this work?", or state why nowhere yet (owner's together-rule, 2026-08-31). The plan also names the adjacent artifacts it allows (tests, migration, doc updates); an unlisted one reads as unasked-for in review. Done: every criterion names its evidence and the allowed-artifacts line exists.
