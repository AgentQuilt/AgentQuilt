...
5. Docs match what shipped: for every part the diff adds, changes or removes, the line that describes it (INDEX.md, AGENTS.md, the roster, a MODULE.md, a docs page) says what the tree now does (map-outdated, `.claude/rules/architecture.md`); `git diff --stat` is the list to walk. Done: each touched part has a current line, or the gap is listed.
