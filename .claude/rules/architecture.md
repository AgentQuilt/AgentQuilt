...
Source: `V=../AgentQuilt-Vault`, `$V/docs/architecture/design-rules.md` and `glossary.md`. Design prose uses module · interface · implementation · depth · seam · adapter · leverage · locality and never "component", "service", "API", "boundary"; "module" is used in one declared sense (kernel module, buildable module, or the design sense). A new domain name gets a glossary entry in the same change.
...
- vocabulary: the word rule above, plus every kernel concept, seam, registry field or ledger event kind carries an ADR number.
