...
Current phase: building the factory (skills, rules, agents) before any product code. Stack (settled): Python/uv, FastAPI, Postgres in Docker, inside WSL2 on ext4.
...
Specs, decisions and memory live in the private prep vault (repo `etcircle/agentquilt-vault`); this repo holds machinery and code. Read order at session start:
