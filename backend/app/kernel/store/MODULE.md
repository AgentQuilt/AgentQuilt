...
- Interface: `session(org_id, principal_id)`, an async context manager, `engine()`, `tenants()` (every org and the system principal a background role acts as in it) and `seed()`.
